...
Génère la structure de dossiers et fichiers décrite dans les arguments.

Si aucun argument n'est fourni, demande à l'utilisateur de décrire la structure souhaitée.

Pour chaque chemin identifié dans $ARGUMENTS, applique ces règles :

- Chemin terminant par / : c'est un dossier, crée-le s'il n'existe pas.
- Chemin sans / final : c'est un fichier, crée les dossiers parents si nécessaire puis crée le fichier.
...
- Nom contenant _model ou placé dans un dossier models/ : classe modèle vide.
- Nom contenant _widget ou placé dans widgets/ ou components/ : StatelessWidget vide.
